package me.portmapping.pbunkers.tablist.layout;
...
import me.portmapping.pbunkers.tablist.protocol.Reflection;
...
public class TabLayout {
  ...
  private final PlayerInteractManager playerInteractManager = new PlayerInteractManager(
      worldServer);

  private final PBunkersTablist instance;

  private final Player player;
  private final EntityPlayer entityPlayer;

  public TabLayout(PBunkersTablist instance, Player player) {
  ...
  public void setHeaderAndFooter() {
    ...
    String header = (ChatColor.translateAlternateColorCodes('&',
        instance.getAdapter().getHeader(player)));
    String footer = (ChatColor.translateAlternateColorCodes('&',
        instance.getAdapter().getFooter(player)));

    IChatBaseComponent headerComponent = ChatSerializer.a(
        "{text:\"" + StringEscapeUtils.escapeJava(header) + "\"}");
    IChatBaseComponent footerComponent = ChatSerializer.a(
        "{text:\"" + StringEscapeUtils.escapeJava(footer) + "\"}");
  ...
  public void update(int column, int row, String text, int ping, Skin skin) {
    if (row > 19) {
      throw new IllegalArgumentException("Row is above 19 " + row);
    }

    if (column > 4) {
      throw new IllegalArgumentException("Column is above 4 " + column);
    }
    ...
    // check prefix char to color code
    if (text.length() > 16) {
    ...
    String teamName = "$" + getTeamAt(row, column); // create a new team
  ...
  private void fetchPing(int index, GameProfile gameProfile, int ping) {
    ...
    // putting a new bars ping
    pingMapping.put(index, ping);

    PacketPlayOutPlayerInfo packetPlayOutPlayerInfo = new PacketPlayOutPlayerInfo(
        EnumPlayerInfoAction.UPDATE_LATENCY, entityPlayer);
  ...
  private void fetchSkin(int index, GameProfile gameProfile, Skin skin) {
    ...
    newGameProfile.getProperties().put(Skin.TEXTURE_KEY, getSkinProperty(skin));

    EntityPlayer entityPlayer = new EntityPlayer(minecraftServer, worldServer, newGameProfile,
        playerInteractManager);
  ...
  private Property getSkinProperty(Skin skin) {
    return new Property(Skin.TEXTURE_KEY, skin.getValue(), skin.getSignature());
  }
  ...
  public void create() {
    ...
    Reflection.getField(packetInfo.getClass(), "a", Object.class)
        .set(packetInfo, EnumPlayerInfoAction.ADD_PLAYER);

    List<PacketPlayOutPlayerInfo.PlayerInfoData> infoDatas = (List<PacketPlayOutPlayerInfo.PlayerInfoData>) Reflection.getField(
        packetInfo.getClass(), "b", Object.class).get(packetInfo);
    ...
    for (int row = 0; row < 20; row++) {
      for (int column = 0; column < 3; column++) {
        ...
        infoDatas.add(
            packetInfo.new PlayerInfoData(gameProfile, 0, WorldSettings.EnumGamemode.SURVIVAL,
                null));

        // default skin bars
        pingMapping.put(index, 0);
        // defualt game profile to bars
    ...
    for (int index = 60; index < 80; index++) {
      ...
      infoDatas.add(
          packetInfo.new PlayerInfoData(gameProfile, 0, WorldSettings.EnumGamemode.SURVIVAL, null));
    ...
    //  modify values of class @{link PacketPlayOutScoreboardTeam}
    ...
    for (int row = 0; row < 20; row++) {
      for (int column = 0; column < 4; column++) {
        ...
        Reflection.getField(packet.getClass(), "e", String.class).set(packet, "always");
        Reflection.getField(packet.getClass(), "g", Object.class)
            .set(packet, Collections.singleton(getTeamAt(row, column)));
    ...
    Reflection.getField(scoreboardTeam.getClass(), "g", Object.class)
        .set(scoreboardTeam, Collections.singleton(player.getName()));
  ...
  private String getTeamAt(int index) {
    return (
        ChatColor.BOLD.toString() +
            ...
            ChatColor.YELLOW +
            (index >= 10 ?
                ChatColor.COLOR_CHAR + String.valueOf(index / 10) + ChatColor.COLOR_CHAR
                    + index % 10 :
  ...
  private boolean isUsingProtocolSupport() {
